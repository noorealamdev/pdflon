(function () {
    ...
    const loadSVG = (canvas) => {
        $('.icons-svg').on('click', 'svg', function (e) {
            canvas.isDrawingMode = false;
            ...
            fabric.loadSVGFromString(svgStr, function (objects, options) {
                ...
                toastr['success']('', 'Item added on the canvas', {
    ...
    // Send object backward and forward
    const setObjectBackFront = (canvas) => {
        let objectToSend;
        canvas.on("selection:created", function(event){
            objectToSend = event.target;
        });
        canvas.on("selection:updated", function(event){
            objectToSend = event.target;
        });

        $('#toolBackObject').click(function() {
            canvas.sendToBack(objectToSend);
        });

        $('#toolFrontObject').click(function() {
            canvas.bringToFront(objectToSend);
        });
    }
